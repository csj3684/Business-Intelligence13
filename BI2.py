    ...
    for s_category in size_category:
        ...
        for size in s_adjusted_user_profile.index:
            if s_adjusted_user_profile.loc[size, 'num'] < num_tolerance:
                ...
                continue
            s_adjusted_user_profile.loc[size, 'sum'] = s_adjusted_user_profile.loc[size, 'numerator'] / \
                                                       s_adjusted_user_profile.loc[size, 'denominator']
...
def get_estimate_size(user_id, distance_matrix, starT_idx, enD_idx, rating_info, clothing_info, size_category):
    similar_user_rating_info = pd.DataFrame(
        columns=['user_id', 'distance', 'influence'] + list(clothing_info.columns[0:4]))
    total_influence = 0
    ...
    for start_idx, end_idx in zip(starT_idx, enD_idx):
        for user_idx in range(start_idx, end_idx + 1):
            ...
            if rating_info.iloc[user_idx].loc[clothing] == 2:  # 적당함 이면
                ...
                similar_user_rating_info.loc[similar_user_rating_info.index.size] = [rating_info.index[user_idx],
                                                                                     distance, None] + size
                if distance == 0:
                    distance = 0.9
    ...
    for s_category in size_category:
        ...
        for user in similar_user_rating_info.loc[:, s_category].index:
            estimate_value += (similar_user_rating_info.loc[user][s_category] * similar_user_rating_info.loc[user][
                'influence'])
...
data = pd.read_csv('C:/Users/csj36/Desktop/Business-Intelligence13/user_table_adjusted.csv', engine='python')
...
data = pd.read_csv('C:/Users/csj36/Desktop/Business-Intelligence13/product_table.csv', engine='python')
...
data = pd.read_csv('C:/Users/csj36/Desktop/Business-Intelligence13/user_product_table_adjusted.csv', engine='python')
...
distance_matrix, start_idx, end_idx, n = get_personal_D_modified(user_info, int(user_id), height_idx_start,
                                                                 height_idx_end, delta_h_tolerance, delta_w_tolerance,
                                                                 required_num)
user_profile_info.loc[user_id] = get_user_profile(user_id, distance_matrix, start_idx, end_idx, rating_info,
                                                  clothing_info, size_category, penalty)
adjusted_user_profile_info.loc[user_id] = get_adjusted_user_profile(user_profile_info.loc[user_id], size_tolerance,
                                                                    num_tolerance)
user_size_info.loc[int(user_id)], log = get_estimate_size(int(user_id), distance_matrix, start_idx, end_idx,
                                                          rating_info, clothing_info, size_category)
...
evaluation = evaluate_by_user_profile(user_id, list(clothing_info.index), adjusted_user_profile_info, clothing_info,
                                      size_category)

print(evaluation.loc[100016, 'compatibility'])

print(evaluation.loc[100016, 'score'])

best_fit = get_best_fit(evaluation)

error = get_error(user_size_info, user_id, '100194', clothing_info, size_category)

print(user_size_info.loc[int(user_id)].loc['length'])
